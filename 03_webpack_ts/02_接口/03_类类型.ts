 ...
 * @LastEditTime: 2021-09-23 13:04:45
 * @FilePath: \ts\03_webpack_ts\02_接口\03_类类型.ts
 */
//类 类型：类的类型，可以通过接口来实现
(()=>{
    //定义一个接口
    interface IFly{
        //定义一个方法，该方法中没有任何的实现(方法中什么都没有) 要求实现这个接口的类需要有fly这个方法
        fly()
    }
    //定·义一个类，这个类的类型就是上面定义的接口(也可以理解为IFly接口约束了这个person类)
